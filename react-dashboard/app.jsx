          ...
          {selectedState ? (
            <div className="bg-navy-800 p-4 rounded-xl border border-indigo-500/50 shadow-lg space-y-2">
              ...
              <div className="text-xs text-slate-300 space-y-1 pt-1">
                ...
                <div className="flex justify-between"><span>Property Thefts:</span><b>{selectedState.property_crimes?.toLocaleString()}</b></div>
              </div>
              <button
              ...
              {regionalDistricts.slice(0, 8).map((d, i) => (
                <div key={i} className="p-2 rounded-lg bg-navy-950/70 border border-slate-800 hover:border-slate-700 transition">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${d.risk_level === 'CRITICAL' ? 'bg-rose-500/20 text-rose-400' : 'bg-amber-500/20 text-amber-400'}`}>
                      {d.risk_level}
                    </span>
                  </div>
...
function PredictorView({ hotspotsData, evalData }) {
  ...
  const calculatePrediction = () => {
    ...
    const score = (
      0.2167 * Math.min(districtRisk / 5000.0, 1.0) +
      0.5289 * Math.min(vRatio / 0.35, 1.0) +
      0.1246 * Math.min(pRatio / 0.40, 1.0) +
      0.0869 * Math.min(wRatio / 0.15, 1.0)
    );
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="pt-4 border-t border-slate-700/60 space-y-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
            <div className="p-2 bg-navy-950 rounded border border-slate-800">
              <span className="text-slate-400">Violent Ratio:</span> <b className="text-rose-400">52.89%</b>
            </div>
            <div className="p-2 bg-navy-950 rounded border border-slate-800">
              <span className="text-slate-400">District Baseline:</span> <b className="text-indigo-400">21.67%</b>
            </div>
            <div className="p-2 bg-navy-950 rounded border border-slate-800">
              <span className="text-slate-400">Property Ratio:</span> <b className="text-amber-400">12.46%</b>
            </div>
            <div className="p-2 bg-navy-950 rounded border border-slate-800">
              <span className="text-slate-400">Women Crime:</span> <b className="text-purple-400">8.69%</b>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-navy-800 p-6 rounded-2xl border border-slate-700/60 shadow-xl flex flex-col justify-between space-y-6">
        ...
        <div className="text-[11px] text-slate-500 text-center border-t border-slate-700/50 pt-3">
          Model Accuracy: <b>70.61%</b> • AUC-ROC: <b>0.9365</b> • Recall: <b>97.85%</b>
        </div>
      </div>
    </div>
  );
